...
export const initializeGuildRelations = (server: ServerState, rng: Rng): ServerState => {
  ...
  for (const from of server.guilds ?? []) {
    for (const to of server.guilds ?? []) {
      ...
      if (current) {
        relations.push({ ...current, value: clampRelation(current.value), lastChangedDay: current.lastChangedDay ?? server.serverDay, lastChangedMinute: current.lastChangedMinute ?? server.currentMinute });
        continue;
      }
      const [min, max] = rangeFor(from.guildFocus ?? 'hybrid', to.guildFocus ?? 'hybrid', rng);
      relations.push({ fromGuildId: from.id, toGuildId: to.id, value: clampRelation(rng.int(min, max) + (rng.chance(0.08) ? rng.int(-35, 35) : rng.int(-12, 12))), lastChangedDay: server.serverDay, lastChangedMinute: server.currentMinute });
    }
  }
...
export const updateGuildRelations = (server: ServerState, rng: Rng): ServerState => ({
  ...
  guildRelations: (server.guildRelations ?? []).map((entry) => {
    const from = server.guilds.find((guild) => guild.id === entry.fromGuildId);
    const to = server.guilds.find((guild) => guild.id === entry.toGuildId);
    let drift = 0;
    ...
    return drift ? { ...entry, value: clampRelation(entry.value + drift), lastChangedDay: server.serverDay, lastChangedMinute: server.currentMinute } : entry;
  }),
});

export const changeGuildRelation = (server: ServerState, fromGuildId: string, toGuildId: string, delta: number): ServerState => ({
  ...server,
  guildRelations: (server.guildRelations ?? []).map((entry) => entry.fromGuildId === fromGuildId && entry.toGuildId === toGuildId ? { ...entry, value: clampRelation(entry.value + delta), lastChangedDay: server.serverDay, lastChangedMinute: server.currentMinute } : entry),
});
