...
import { canRegisterPlayerGuildForCastle } from '../../systems/siegeSystem';
...
export const CastlePanel = () => {
  ...
  const castles = useMemo(() => [...(server.castles ?? [])].sort((a, b) => a.tier.localeCompare(b.tier) || a.name.localeCompare(b.name)), [server.castles]);
  ...
  return (
    <div className="screen-stack">
      <section className="panel hero-panel">
        ...
        <h1>Осады замков</h1>
        <p className="muted">Мид и хай гильдии регистрируют составы 5–10 человек. Победитель осады становится владельцем замка.</p>
      </section>

      {currentRun && (
        <section className="panel">
          <div className="section-title">Активная / последняя осада</div>
          <div className="title-row">
            <h2>{castles.find((castle) => castle.id === currentRun.castleId)?.name ?? currentRun.castleId}</h2>
            <strong>Ход {currentRun.turn} · {currentRun.status}</strong>
          </div>
          <SiegeMapView run={currentRun} />
          <div className="list-lines compact-list">
            {currentRun.units.slice(0, 16).map((unit) => (
          ...
          <div className="combat-log">{currentRun.log.slice(-12).map((line, index) => <div key={`${line}_${index}`}>{line}</div>)}</div>
        </section>
      )}
          ...
          {castles.map((castle) => {
            ...
            return (
              <article key={castle.id} className="content-card guild-card">
                ...
                <span>Следующая осада: {timeText(castle.nextSiegeDay, castle.nextSiegeMinute)}</span>
                <span>Зарегистрировано: {(castle.registeredGuildIds ?? []).length} гильдий</span>
                {roster && <span className="ready-line">Твой состав: {roster.memberIds.length}/10</span>}
                <div className="action-grid compact-actions">
                  {roster
                    ? <button onClick={() => unregisterSiegeRoster(castle.id)}>Снять регистрацию</button>
                    : <button disabled={!check.ok} onClick={() => registerSiegeRoster(castle.id)}>{check.ok ? 'Зарегистрировать авто-состав' : check.reason}</button>}
                </div>
                {roster && (
                  <div className="list-lines compact-list">
