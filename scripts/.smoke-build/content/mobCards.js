import { calculateCardPrice } from '../balance/index.js';
import { rarityScore } from './itemFinalize.js';
export const getMobCardId = (mob) => `card_${mob.id}`;
export const getMobCardRarity = (mob) => {
    if (mob.id === 'first_wyrm')
        return 'legendary';
    if (mob.tags.includes('raid') && mob.tags.includes('boss'))
        return 'legendary';
    if (mob.tags.includes('boss'))
        return 'epic';
    if (mob.tags.includes('dungeon') || mob.tags.includes('elite') || mob.tags.includes('mini-boss'))
        return 'epic';
    return 'rare';
};
export const getMobCardStats = (mob) => {
    if (mob.id === 'first_wyrm')
        return { hp: 160, defense: 20, attack: 20, magic: 14 };
...
export const getMobCardDropChance = (mob) => {
    if (mob.tags.includes('raid') && mob.tags.includes('boss'))
        return 0.000006;
    if (mob.tags.includes('boss'))
        return 0.00001;
...
export const createMobCard = (mob) => {
    const card = {
        id: getMobCardId(mob),
        name: `Карта: ${mob.name}`,
        type: 'card',
...
export const createMobCardsForMobs = (mobs) => {
    const byId = new Map();
