const nearestLevel = (level, pool) => pool.reduce((best, current) => Math.abs(current - level) < Math.abs(best - level) ? current : best, pool[0]);
const legacyCardIds = {
...
export const normalizeLegacyItemId = (id) => {
    ...
    if (thornCrypt)
        return `old_lantern_${thornCrypt[1]}_${thornCrypt[2]}`;
    const glassTrimmed = id.match(/^glass_catacomb_(warrior|ranger|mage|priest)_(head|boots)$/);
    if (glassTrimmed)
        return `glass_catacomb_${glassTrimmed[1]}_${glassTrimmed[2] === 'head' ? 'chest' : 'legs'}`;
...
export const normalizeLegacySetId = (setId) => {
    if (!setId)
        return setId;
    if (setId === 'dungeon_thorn_crypt')
        return 'dungeon_old_lantern';
    if (setId === 'dungeon_glass_catacomb_epic')
        return 'dungeon_glass_catacomb';
    if (/^epic_(warrior|ranger|mage|priest)_/.test(setId))
        return setId.replace(/^epic_/, 'rare_');
    return setId;
};
