import { calculateCardPrice, calculateItemPrice } from '../balance/index.js';
import { normalizeLegacyItemId, normalizeLegacySetId } from './itemLegacy.js';
const socketSlotsFor = (item) => {
    if (!item.slot || item.type === 'card' || item.type === 'consumable' || item.type === 'material' || item.type === 'quest')
        return 0;
    if (item.rarity === 'legendary' || item.rarity === 'epic')
        return 2;
    if (item.rarity === 'rare')
        return Math.max(1, item.socketSlots ?? 1);
    return Math.max(0, item.socketSlots ?? 0);
};
const sourceFor = (item) => {
    if (item.sourceType)
        return { sourceType: item.sourceType, sourceId: item.sourceId, sourceName: item.sourceName };
    if (item.setId?.startsWith('raid_'))
        return { sourceType: 'raid', sourceId: item.sourceId, sourceName: item.sourceName ?? 'Рейд' };
    if (item.setId?.startsWith('dungeon_'))
        return { sourceType: 'dungeon', sourceId: item.sourceId, sourceName: item.sourceName ?? 'Данж' };
    if (item.setId)
        return { sourceType: 'general', sourceId: item.sourceId ?? 'general_sets', sourceName: item.sourceName ?? 'Общий сет' };
