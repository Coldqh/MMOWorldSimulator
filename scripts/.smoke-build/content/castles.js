const buildMap = (id, variant) => {
    ...
    for (let y = 0; y < 10; y += 1) {
        for (let x = 0; x < 10; x += 1) {
            ...
            if (border || crossWall || mazeWall)
                type = 'wall';
            if ((x === 1 && y === 1) || (x === 8 && y === 8) || (x === 1 && y === 8) || (x === 8 && y === 1))
                type = 'spawn';
            if ((x === 4 || x === 5) && (y === 4 || y === 5))
                type = 'center';
            if ((x === 4 && y === 1) || (x === 5 && y === 8) || (x === 1 && y === 4) || (x === 8 && y === 5))
                type = 'gate';
            if ((x === 2 && y === 2) || (x === 7 && y === 7))
                type = 'tower';
            cells.push({ x, y, type });
...
export const CASTLE_SIEGE_WEEKDAYS = {
    virspire_citadel: 0,
    glass_crown_fortress: 2,
    dragonspire_hold: 4,
};
export const getSiegeMapById = (id) => SIEGE_MAPS.find((map) => map.id === id) ?? SIEGE_MAPS[0];
