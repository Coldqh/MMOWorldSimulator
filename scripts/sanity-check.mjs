...
if (gsBad.length || /\n\s*import\s/.test(gsBody)) {
  console.error('Sanity failed: GameStore import sanitizer checks');
...
if (!gsHead.includes('import { create } from "zustand";') || !gsHead.includes('SAVE_VERSION,') || !gsHead.includes('startCurrentSiege')) {
  console.error('Sanity failed: clean required imports missing');
  process.exit(1);
}
console.log('Sanity passed: GameStore import sanitizer checks');
